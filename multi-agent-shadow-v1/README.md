...
`live_runner.py`는 OpenAI Agents SDK를 사용해 선택된 후보를 네 번 독립 심사합니다.
...
기본 시험은 후보 1건, 최대 4회 모델 호출입니다. 후보는 최대 2건, 호출은 최대 8회로 코드에서 제한합니다. 각 단계는 구조화된 공통 출력 계약을 통과해야 하며, 인용문·출처 ID·URL이 실제 입력과 일치하지 않으면 전체 실행을 실패시킵니다.
...
결과는 30일 보관되는 별도 artifact에만 저장되며, 공식 브리핑·질문 장부·검증 장부에는 반영되지 않습니다. Agents SDK의 추적도 꺼서 입력과 판단 결과가 별도 추적 로그로 전송되지 않도록 했습니다.
