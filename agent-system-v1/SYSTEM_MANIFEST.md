...
- 개별 원문·상세 페이지 날짜를 기준으로 소스 주기별 신선도 창을 적용한다.
- 같은 원문 지문은 다음 실행부터 `동일 원문 재등장`으로 분리하며 오늘 새 카드와 자동 전이에서 제외한다.
- 신선도 초과 자료는 `STALE_CARRYOVER`, 날짜가 확인되지 않은 자료는 `날짜 확인 대기`로 분리한다.
- 전국 단서는 `서울 지역화 대기`에 두고 서울 원자료가 확보되기 전에는 편집 카드나 S0 제안을 만들지 않는다.
- 접속 실패와 본문 저하를 최종 브리핑에 표시해 0건을 현상 부재로 오해하지 않게 한다.
- 실행 묶음의 모든 허용 산출물과 이번 실행 history에 SHA-256 지문을 부여하고, main 저장 전후에 검증한다.
