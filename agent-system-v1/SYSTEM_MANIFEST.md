...
18. 자치구의회 고정 7건 L3 발언 문맥 그림자 검토: L2에서 비임시본과 회의일 대응이 확인된 문서만 고정한다. PR은 원문 접근·본문 변화만 검사하고, 수동 실행만 최대 7회 모델 호출로 전체 발언 문맥에서 회의록당 0~1건의 검증 질문을 도출한다. 원문 전체는 비저장, 결과는 7일 artifact로만 제공한다. 구의회 성숙도는 L1로 유지하고 편집 v4·공개 브리핑·장부로 자동 연결하지 않는다.
