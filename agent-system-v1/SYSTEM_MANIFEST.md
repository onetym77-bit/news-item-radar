...
현재 일일 신규 소스 입력과 브리핑의 실행기는 GitHub Actions다. 읽기 전용 작업에서 소스를 한 번만 수집·검증해 artifact로 넘기고, main 전용 쓰기 작업만 그 동일 산출물을 저장한다. PR, 다른 브랜치, 과거 날짜 재실행에는 쓰기 권한과 Git 자격증명을 주지 않으며 main 최신본을 덮어쓰지 않는다.
